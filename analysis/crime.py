...
couch = couchdb.Server('http://cadmin:qwerty8888@115.146.95.52:9584/')

sourcedb = couch['tweet_raw']

target_time = '2017-01-01 00:00:00'
current_time = strftime("%Y-%m-%d %H:%M:%S", gmtime())

one_days_ago = datetime.now() - timedelta(days=1)
date = str(one_days_ago).split(' ')[0]
...
def main():
    ...
    for tweet in final_data:
        ...
        if datetime > date_start and datetime < date_end:
            ...
            pass
    try:
        targetdb = couch['result_tweet_raw_trump']
    except couchdb.http.ResourceNotFound:
        targetdb = couch.create('result_tweet_raw_trump')
